...
namespace UnityEngine.XR.ARFoundation.Samples
{
    ...
    public class ManualEnvironmentProbeController : MonoBehaviour
    {
        [SerializeField]
        [Tooltip(
            "Reference to the AREnvironmentProbe prefab to be placed manually by this controller"
        )]
        AREnvironmentProbe m_ProbePrefab;

        [SerializeField]
        [Tooltip(
            "Reference to the AREnvironmentProbeManager used to query the current platform's subsystem descriptor for supported features"
        )]
        ...
        [SerializeField]
        [Tooltip(
            "Reference to the button that creates a manual probe at the camera position. It is interactable only if manual placement is supported"
        )]
        Button m_CreateButton;

        [SerializeField]
        [Tooltip(
            "Reference to the canvas group of probe destroy button options. They are interactable only if manual removal is supported"
        )]
        ...
        /// </summary>
        public bool canManuallyPlace =>
            m_ProbePrefab != null && m_ProbeManager.descriptor.supportsManualPlacement;
        ...
        Pose originCameraPose =>
            new(m_OriginCameraTransform.position, m_OriginCameraTransform.rotation);
        ...
        public AREnvironmentProbe PlaceProbeAt(Pose pose) =>
            canManuallyPlace ? InstantiateProbe(pose) : null;
