...
namespace UnityEngine.XR.ARFoundation.Samples
{
    ...
    {
        [Tooltip(
            "The UI Text element used to display the state of the Plane Detection Mode flags."
        )]
        [SerializeField]
        TextMeshProUGUI m_FlagsText;

        [Tooltip(
            "The SliderToggle element used to control the on/off state of Horizontal plane detection."
        )]
        [SerializeField]
        SliderToggle m_HorizontalSliderToggle;

        [Tooltip(
            "The SliderToggle element used to control the on/off state of Vertical plane detection."
        )]
        [SerializeField]
        SliderToggle m_VerticalSliderToggle;

        [Tooltip(
            "The SliderToggle element used to control the on/off state of NotAxisAligned plane detection."
        )]
        [SerializeField]
        ...
        void Start()
        {
            ...
            if (m_ARPlaneManager.descriptor.supportsHorizontalPlaneDetection)
                m_HorizontalSliderToggle.SetSliderValue(
                    (detectionMode & PlaneDetectionMode.Horizontal) == PlaneDetectionMode.Horizontal
                );
            else
                m_HorizontalSliderToggle.SetEnabled(false);

            if (m_ARPlaneManager.descriptor.supportsVerticalPlaneDetection)
                m_VerticalSliderToggle.SetSliderValue(
                    (detectionMode & PlaneDetectionMode.Vertical) == PlaneDetectionMode.Vertical
                );
            else
                m_VerticalSliderToggle.SetEnabled(false);

            if (m_ARPlaneManager.descriptor.supportsArbitraryPlaneDetection)
                m_NotAxisAlignedSliderToggle.SetSliderValue(
                    (detectionMode & PlaneDetectionMode.NotAxisAligned)
